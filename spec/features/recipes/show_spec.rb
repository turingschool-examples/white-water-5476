...
RSpec.describe "recipes show page", type: :feature do
  ...
  it "has a form to add an ingredient to the recipe" do
    ...
    herbals = spaghet.ingredients.create!(name: "Herbs", cost: 1)
    sauce = spaghet.ingredients.create!(name: "Red Sauce", cost: 5)
    anchovies = Ingredient.create!(name: "Anchovies", cost: 4)

    visit "/recipes/#{spaghet.id}"

    fill_in "New", with: "#{anchovies.id}"
    click_button "Add Ingredient ID"

    expect(current_path).to eq("/recipes/#{spaghet.id}")
    expect(page).to have_content("Anchovies")
    expect(page).to have_content("Total Cost of Dish: $15")
  end
end
